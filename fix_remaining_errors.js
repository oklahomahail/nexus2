...
function fixRemainingIssues(content, filename) {
  ...
  fixedContent = fixedContent.replace(
    ...
    (match, imports, from) => {
      ...
      const fixedImports = imports.replace(
        /\b(DonorInsights|Analytics|Campaign)\b/g,
        "_$1 as $1",
      );
  ...
  if (filename.includes("/components/") && filename.endsWith(".tsx")) {
    ...
    if (
      !fixedContent.includes(`interface ${propsInterfaceName}`) &&
...
`;
      ...
      if (lastImportIndex !== -1) {
        ...
        fixedContent =
          fixedContent.slice(0, nextLineIndex + 1) +
          interfaceDefinition +
  ...
  fixedContent = fixedContent.replace(/(\w+)\s*=>\s*\(/g, "($1: any) => (");
