...
function getAllTsFiles(dir = "src", fileList = []) {
  try {
    ...
    if (!fs.existsSync(dir)) {
      ...
      if (!fs.existsSync(dir)) {
        console.log(
          "src directory not found. Make sure you're running this from the project root or scripts folder.",
        );
...
function fixUnderscoreProps(content) {
  ...
  Object.entries(propMappings).forEach(([underscore, standard]) => {
    ...
    const standaloneVarRegex = new RegExp(
      `\\b${varName}\\b(?![A-Za-z0-9_])`,
      "g",
    );
...
function addMissingInterfaces(content, filename) {
  ...
  if (
    ...
    const functionMatch = content.match(
      new RegExp(
        `function\\s+${componentName}\\s*\\([^)]*\\)|const\\s+${componentName}\\s*[=:]\\s*\\([^)]*\\)\\s*=>`,
      ),
    );
