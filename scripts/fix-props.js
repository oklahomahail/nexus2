...
function fixUnderscoreProps(content) {
  ...
  Object.entries(propMappings).forEach(([underscore, standard]) => {
    ...
    const destructuringRegex = new RegExp(`\\b${underscore.replace('_', '_')}\\b(?=\\s*[,}])`, 'g');
    fixedContent = fixedContent.replace(destructuringRegex, standard);
    
    // Fix property definitions in interfaces/types
    const propDefRegex = new RegExp(`^(\\s*)${underscore.replace('_', '_')}(\\??\\s*:)`, 'gm');
  ...
  fixedContent = fixedContent.replace(/Cannot find name 'onCreateCampaign'/g, '');
...
function main() {
  ...
  console.log('   - Any remaining type errors specific to your component logic');
}
...
module.exports = { fixUnderscoreProps, fixDonorInsightsImport, fixMissingPropsInComponents };