...
const reactImportRegex = /^\s*import\s+React(?:\s*,\s*\{([^}]*)\})?\s+from\s+['"]react['"];?\s*$/m;
...
write(filePath, src);
console.log(`✅ Converted imports to lazy in ${path.relative(process.cwd(), filePath)}.`);
console.log(`ℹ️  A backup was saved as ${path.relative(process.cwd(), filePath + '.bak')}.`);
