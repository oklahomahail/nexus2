...
export default function ExampleDashboard() {
  return (
    <div className="px-8 py-10 editorial-flow">
      ...
      {/* Key Metrics Section */}
      <SectionBlock title="Key Metrics" description="Overview of campaign performance">
...
function MetricCard({ label, value, change, trend }: MetricCardProps) {
  return (
    <div className="p-4 bg-[var(--nx-bg-secondary)] border border-[var(--nx-border)] rounded-[var(--nx-radius-md)]">
      <div className="text-[13px] text-[var(--nx-text-muted)] mb-1">{label}</div>
      <div className="text-[32px] font-semibold text-[var(--nx-charcoal)] mb-2">{value}</div>
      <div className={`text-[13px] font-medium ${trend === "up" ? "text-[var(--nx-success)]" : "text-[var(--nx-error)]"}`}>
  ...
  return (
    <div className="flex items-start gap-3 p-3 hover:bg-[var(--nx-bg-secondary)] rounded-[var(--nx-radius-sm)] transition-colors">
      ...
      <div className="flex-1">
        <div className="text-[15px] font-medium text-[var(--nx-charcoal)]">{title}</div>
        <div className="text-[13px] text-[var(--nx-text-muted)]">{description}</div>
      </div>
...
function CampaignCard({ name, raised, goal, progress, daysRemaining }: CampaignCardProps) {
  return (
    <div className="p-5 border border-[var(--nx-border)] rounded-[var(--nx-radius-md)] hover:shadow-[var(--nx-shadow-md)] transition-shadow">
      <h3 className="text-[18px] font-semibold text-[var(--nx-charcoal)] mb-2">{name}</h3>

      <div className="flex items-baseline gap-2 mb-3">
        <span className="text-[24px] font-semibold text-[var(--nx-charcoal)]">{raised}</span>
        <span className="text-[13px] text-[var(--nx-text-muted)]">of {goal}</span>
      </div>
      ...
      <div className="flex items-center justify-between text-[13px]">
        <span className="text-[var(--nx-text-muted)]">{progress}% complete</span>
        <span className="text-[var(--nx-text-muted)]">{daysRemaining} days remaining</span>
      </div>
    </div>
  );
}
