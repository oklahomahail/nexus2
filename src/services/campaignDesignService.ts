...
import { estimatePostage, MailClass, MailFormat } from '@/services/postageEstimator'
...
export async function loadBrandContext(
  ...
  const [{ data: profile, error: e1 }, { data: corpus, error: e2 }] = await Promise.all([
    supabase
      .from('brand_profiles')
      .select(
  ...
  return {
    ...
    snippets: (corpus || []).map((c: any) => ({ title: c.title, content: c.content })),
  }
}
...
export function buildPrompts(ctx: BrandContext, params: CampaignParams) {
  const ctxText = [
    ...
    ctx.profile.mission_statement ? `MISSION: ${ctx.profile.mission_statement}` : '',
    ctx.profile.tone_of_voice ? `TONE: ${ctx.profile.tone_of_voice}` : '',
    ctx.profile.brand_personality ? `PERSONALITY: ${ctx.profile.brand_personality}` : '',
  ...
  const snippets = ctx.snippets
    .map((s, i) => `• ${s.title ?? 'Snippet ' + (i + 1)}: ${s.content.slice(0, 400)}`)
    .join('\n')
