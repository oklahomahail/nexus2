...
export function compareMailClasses(opts: {
  ...
  const nonprofit = estimatePostage({
    mailClass: "nonprofit",
    format,
    quantity,
  });
  const firstClass = estimatePostage({
...
export function estimateCampaignMailing(opts: {
  ...
  const printingPerPiece =
    opts.printingCostPerPiece ?? defaultPrintingCosts[format];
  const productionPerPiece =
    opts.productionCostPerPiece ?? defaultProductionCosts[format];
