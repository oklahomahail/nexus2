 ...
 */
export async function listBrandProfiles(clientId: string): Promise<BrandProfile[]> {
 ...
 */
export async function getPrimaryBrandProfile(clientId: string): Promise<BrandProfile | null> {
  const { data, error } = await supabase
    .from('brand_profiles')
    .select('*')
    .eq('client_id', clientId)
 ...
 * Add a brand asset
 */
export async function addBrandAsset(input: BrandAssetInput): Promise<BrandAsset> {
  const { data, error } = await supabase
    .from('brand_assets')
    .insert(input)
    .select('*')
 ...
 */
export async function upsertCorpusChunk(input: BrandCorpusInput): Promise<BrandCorpusChunk> {
  const { data, error } = await supabase
    .from('brand_corpus')
    .upsert(input, {
...
export async function getTopCorpusSnippets(
  ...
  return data.map((chunk: any) => ({
    ...
    snippet: chunk.content.slice(0, maxCharsPerSnippet) + (chunk.content.length > maxCharsPerSnippet ? '...' : ''),
  }))
}
