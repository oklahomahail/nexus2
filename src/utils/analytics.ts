...
type TutorialEvent = { type: "tutorial_step"; id: string; action: "next" | "back" | "skip" | "dismiss" };
type ClientEvent = { type: "client_switch"; clientId: string };
type CampaignEvent = { type: "campaign_switch"; campaignId: string; clientId: string };

type AnalyticsEvent = NavigationEvent | TutorialEvent | ClientEvent | CampaignEvent;
...
export const analytics = {
  ...
  tutorialStep: (id: string, action: TutorialEvent["action"]) => track({ type: "tutorial_step", id, action }),
  clientSwitch: (clientId: string) => track({ type: "client_switch", clientId }),
  campaignSwitch: (campaignId: string, clientId: string) => track({ type: "campaign_switch", campaignId, clientId }),
