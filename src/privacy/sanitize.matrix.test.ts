...
const piiCases: Array<{ name: string; input: string; redacted: RegExp }> = [
  { name: 'email', input: 'Reach me at a.b+c@ex-ample.co.uk', redacted: /\[EMAIL\]/ },
  {
    name: 'US phone',
  ...
  { name: 'Credit card', input: 'Card 4242 4242 4242 4242', redacted: /\[CARD\]/ },
];
...
describe('sanitizeContent – full pipeline', () => {
  ...
  it('respects option flags', () => {
    ...
    const noHTML = sanitizeContent(input, { stripHTML: false, redactPII: false });
    expect(noHTML.content).toContain('<div>');
    expect(noHTML.content).toContain('test@example.com');

    const withAll = sanitizeContent(input, { stripHTML: true, redactPII: true });
