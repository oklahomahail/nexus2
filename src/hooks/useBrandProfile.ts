...
interface UseBrandProfileReturn {
  ...
  uploadAsset: (input: Omit<BrandAssetInput, 'client_id' | 'brand_id'>) => Promise<void>
  removeAsset: (assetId: string) => Promise<void>

  // Corpus operations
  loadCorpus: () => Promise<void>
  addCorpusEntry: (input: Omit<BrandCorpusInput, 'client_id' | 'brand_id'>) => Promise<void>
...
export function useBrandProfile(clientId: string): UseBrandProfileReturn {
  ...
  const loadProfile = useCallback(async () => {
    ...
    try {
      ...
      const message = err instanceof Error ? err.message : 'Failed to load brand profile'
      setError(message)
  ...
  const loadCorpus = useCallback(async () => {
    ...
    try {
      const corpusChunks = await listBrandCorpus(clientId, profile.id, { limit: 100 })
      setCorpus(corpusChunks)
    } catch (err) {
      const message = err instanceof Error ? err.message : 'Failed to load brand corpus'
      setError(message)
  ...
  const addCorpusEntry = useCallback(
    async (input: Omit<BrandCorpusInput, 'client_id' | 'brand_id'>) => {
      ...
      try {
        ...
        const checksum = input.checksum || (await generateChecksum(input.content))
        ...
        const message = err instanceof Error ? err.message : 'Failed to add corpus entry'
        setError(message)
  ...
  const removeCorpusEntry = useCallback(async (corpusId: string) => {
    ...
    try {
      ...
      const message = err instanceof Error ? err.message : 'Failed to delete corpus entry'
      setError(message)
