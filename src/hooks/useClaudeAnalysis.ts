...
export function useClaudeAnalysis(): UseClaudeAnalysisReturn {
  ...
  const generateNarrative = useCallback(
    async (metric: MetricType, data: any) => {
      setIsLoading(true);
      setError(null);
      ...
      try {
        ...
        if (invokeError) {
          throw new Error(
            invokeError.message || "Failed to generate narrative",
          );
        }

        if (!response || !response.ok) {
          const reason =
            response?.blocked_reason || response?.error || "Unknown error";
        ...
        const text =
          response.data.content?.[0]?.text || "No narrative generated";
        setNarrative(text);
      } catch (err) {
        const message =
