...
export const Topbar: React.FC = () => {
  ...
  if (loc.pathname.includes("/campaigns"))
    crumbs.push({ label: "Campaigns", to: `/clients/${clientId}/campaigns` });
  if (loc.pathname.includes("/analytics"))
    crumbs.push({ label: "Analytics", to: `/clients/${clientId}/analytics` });
  if (loc.pathname.includes("/reports"))
    crumbs.push({ label: "Reports", to: `/clients/${clientId}/reports` });

  return (
    <div className="h-full flex items-center gap-3 px-4">
      <nav
        className="flex items-center gap-2 text-sm"
        data-tutorial-step="top.breadcrumbs"
      >
        {crumbs.map((c, i) => (
