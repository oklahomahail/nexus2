...
const generateCampaignAnalytics = (_campaignId: string) => {
  ...
  const timeSeriesData = Array.from({ length: 25 }, (_, i) => {
    ...
    return {
      ...
      donors: Math.round((i + 1) * (donorCount / 25) * (0.8 + Math.random() * 0.4)),
...
// Component
const CampaignAnalyticsDetail: React.FC<{ _campaignId: string }> = ({ campaignId }) => {
  const [activeTab, setActiveTab] = useState('performance');
  const analytics = useMemo(() => generateCampaignAnalytics(campaignId), [campaignId]);
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-400">Deep performance insights and forecasting</p>
        </div>
        ...
        <div className="flex items-center space-x-3">
          <div className="text-right">
            ...
            <div className={`text-xl font-bold ${getHealthColor(analytics.healthScore.overall)}`}>
      ...
      <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              <div className="bg-slate-800/50 border border-slate-700/50 rounded-xl p-6">
                <div className="flex items-center justify-between mb-4">
                  <h3 className="text-slate-400 text-sm font-medium">Daily Average</h3>
                ...
                <div className={`flex items-center mt-2 text-sm ${
                  analytics.velocity.weeklyTrend > 0 ? 'text-green-400' : 'text-red-400'
                }`}>
                  {analytics.velocity.weeklyTrend > 0 ? <TrendingUp className="w-4 h-4 mr-1" /> : <TrendingDown className="w-4 h-4 mr-1" />}
              ...
              <div className="bg-slate-800/50 border border-slate-700/50 rounded-xl p-6">
                ...
                <div className={`text-sm mt-2 ${
                  analytics.velocity.isOnTrack ? 'text-green-400' : 'text-yellow-400'
                }`}>
                  {analytics.velocity.isOnTrack ? (
                    <><CheckCircle className="w-4 h-4 inline mr-1" />On track</>
                  ) : (
                    <><AlertTriangle className="w-4 h-4 inline mr-1" />Behind pace</>
                  )}
                </div>
              </div>
              ...
              <div className="bg-slate-800/50 border border-slate-700/50 rounded-xl p-6">
                <div className="flex items-center justify-between mb-4">
                  <h3 className="text-slate-400 text-sm font-medium">Projected End</h3>
              ...
              <div className="bg-slate-800/50 border border-slate-700/50 rounded-xl p-6">
                <div className="flex items-center justify-between mb-4">
                  <h3 className="text-slate-400 text-sm font-medium">Momentum</h3>
              ...
              <div style={{ height: 300 }}>
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={[...analytics.timeSeriesData, 
                    { date: 'Goal', cumulative: 50000, daily: 0, donors: 150, avgGift: 250 }
                  ]}>
                    <CartesianGrid strokeDasharray="3 3" stroke="#334155" />
                    <XAxis dataKey="date" stroke="#64748B" fontSize={12} />
                    <YAxis stroke="#64748B" fontSize={12} />
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              {analytics.channels.map((channel, _index) => (
                <div key={channel.channel} className="bg-slate-800/50 border border-slate-700/50 rounded-xl p-6">
                  <div className="flex items-center justify-between mb-4">
                    <h3 className="text-white font-medium">{channel.channel}</h3>
                    <div className={`flex items-center text-sm ${
                      channel.trend > 0 ? 'text-green-400' : 'text-red-400'
                    }`}>
                      {channel.trend > 0 ? <TrendingUp className="w-4 h-4 mr-1" /> : <TrendingDown className="w-4 h-4 mr-1" />}
                  ...
                  <div className="space-y-3">
                    <div>
                      ...
                      <div className="text-lg font-bold text-white">{formatCurrency(channel.revenue)}</div>
                    </div>
                    <div className="grid grid-cols-2 gap-3 text-sm">
                      <div>
                        <div className="text-slate-400 text-xs">Donors</div>
                        <div className="text-white font-medium">{channel.donors}</div>
                      </div>
                      <div>
                        <div className="text-slate-400 text-xs">Avg Gift</div>
                        <div className="text-white font-medium">{formatCurrency(channel.averageGift)}</div>
                      </div>
                      <div>
                        <div className="text-slate-400 text-xs">Conv Rate</div>
                        <div className="text-white font-medium">{channel.conversionRate}%</div>
                      </div>
                      <div>
                        <div className="text-slate-400 text-xs">ROI</div>
                        <div className={`font-medium ${channel.roi > 10 ? 'text-green-400' : channel.roi > 5 ? 'text-yellow-400' : 'text-red-400'}`}>
                          {channel.roi}x
                        </div>
                      </div>
                    </div>
            ...
            <div className="bg-slate-800/50 border border-slate-700/50 rounded-xl p-6">
              <h3 className="text-lg font-semibold text-white mb-4">Channel ROI Comparison</h3>
              <div style={{ height: 300 }}>
                <ResponsiveContainer width="100%" height="100%">
        ...
        {activeTab === 'donors' && (
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <div className="bg-slate-800/50 border border-slate-700/50 rounded-xl p-6">
                <h3 className="text-lg font-semibold text-white mb-4">Donor Distribution</h3>
                <div style={{ height: 300 }}>
                  <ResponsiveContainer width="100%" height="100%">
                    <RechartsPieChart>
                      <Tooltip 
                      ...
                      />
                      <RechartsPieChart data={analytics.segments.map((seg, i) => ({ ...seg, fill: SEGMENT_COLORS[i] }))}>
              ...
              <div className="bg-slate-800/50 border border-slate-700/50 rounded-xl p-6">
                <h3 className="text-lg font-semibold text-white mb-4">Segment Analysis</h3>
                <div className="space-y-4">
                  {analytics.segments.map((segment, _index) => (
                    <div key={segment.segment} className="p-4 bg-slate-700/50 rounded-lg">
                      <div className="flex items-center justify-between mb-2">
                        ...
                        <span className={`text-xs px-2 py-1 rounded ${
                          segment.growthPotential === 'high' ? 'bg-green-500/20 text-green-400' :
                          segment.growthPotential === 'medium' ? 'bg-yellow-500/20 text-yellow-400' :
                      ...
                      <div className="grid grid-cols-2 gap-4 text-sm">
                        <div>
                          ...
                          <div className="text-white font-medium">{segment.count} ({segment.percentage}%)</div>
                        </div>
                        <div>
                          <div className="text-slate-400">Revenue</div>
                          <div className="text-white font-medium">{formatCurrency(segment.revenue)}</div>
                        </div>
                        <div>
                          <div className="text-slate-400">Avg Gift</div>
                          <div className="text-white font-medium">{formatCurrency(segment.averageGift)}</div>
                        </div>
                        <div>
                          <div className="text-slate-400">Retention</div>
                          <div className="text-white font-medium">{segment.retentionLikelihood}%</div>
                        </div>
                      </div>
                    </div>
                  ))}
        ...
        {activeTab === 'health' && (
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              {[
                { label: 'Fundraising', score: analytics.healthScore.fundraising, icon: DollarSign },
                { label: 'Engagement', score: analytics.healthScore.engagement, icon: Users },
                { label: 'Momentum', score: analytics.healthScore.momentum, icon: TrendingUp },
                { label: 'Efficiency', score: analytics.healthScore.efficiency, icon: Target }
              ].map((metric) => (
                <div key={metric.label} className="bg-slate-800/50 border border-slate-700/50 rounded-xl p-6">
                  <div className="flex items-center justify-between mb-4">
            ...
            {/* Health Factors */}
            <div className="bg-slate-800/50 border border-slate-700/50 rounded-xl p-6">
              <h3 className="text-lg font-semibold text-white mb-4">Performance Factors</h3>
              <div className="space-y-4">
                {analytics.healthScore.factors.map((factor, _index) => (
                  <div key={index} className="p-4 bg-slate-700/30 rounded-lg">
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <div className="flex items-center space-x-2">
                        <div className={`text-sm font-medium ${getHealthColor(factor.score)}`}>
                          {factor.score}/100
                        </div>
                    ...
                    <p className="text-slate-400 text-sm">{factor.suggestion}</p>
                  </div>
                ))}
              </div>
            </div>
