...
const reportTemplates: ReportTemplate[] = [
  {
    ...
    sections: [
      ...
      },
      {
        ...
        type: "chart",
        required: false,
        description: "Visual trend charts",
      },
      {
        id: "recommendations",
  ...
  },
  {
    ...
    sections: [
      ...
      },
      {
        ...
        type: "table",
        required: true,
        description: "Donor breakdown and segmentation",
      },
      {
        id: "timeline",
    ...
    sections: [
      {
        ...
        type: "narrative",
        required: true,
        description: "Mission impact achieved",
      },
      {
        id: "donor_recognition",
        title: "Donor Recognition",
        type: "table",
        required: true,
        description: "Donor acknowledgments",
      },
      {
        id: "financials",
        ...
        type: "metrics",
        required: true,
        description: "How funds were used",
      },
      {
        id: "stories",
        ...
        type: "narrative",
        required: false,
        description: "Beneficiary stories and testimonials",
      },
    ],
  },
  {
    ...
    sections: [
      ...
      },
      {
        ...
        type: "table",
        required: true,
        description: "Marketing and operational costs",
      },
      {
        id: "net_analysis",
        title: "Net Impact Analysis",
        type: "metrics",
        required: true,
        description: "Net fundraising results",
      },
    ],
  },
];
...
const ReportGenerator: React.FC = () => {
  ...
  const clientCampaigns = useMemo(() => {
    ...
    const cid = (currentClient as any)?.id;
    return cid ? list.filter((c: any) => (c as any).clientId === cid) : list;
  ...
  const reportStats = useMemo(() => {
    const filteredCampaigns = clientCampaigns.filter((campaign: any) => {
      ...
      const matchesStatus =
        reportConfig.filters.status.length === 0 ||
        reportConfig.filters.status.includes(campaign.status);
      const matchesType =
        reportConfig.filters.campaignTypes.length === 0 ||
        reportConfig.filters.campaignTypes.includes(campaign.type);
      const matchesId =
        reportConfig.filters.campaignIds.length === 0 ||
    ...
    const totalRaised = filteredCampaigns.reduce(
      (sum: number, c: Campaign) => sum + c.raised,
      0,
    );
    const totalGoal = filteredCampaigns.reduce(
      (sum: number, c: Campaign) => sum + c.goal,
      0,
    );
    const totalDonors = filteredCampaigns.reduce(
      (sum: number, c: any) => sum + (c.donorCount ?? 0),
      0,
    );
    const avgProgress =
      filteredCampaigns.length > 0
        ? filteredCampaigns.reduce(
            (sum: number, c: Campaign) =>
              sum + (c.goal ? (c.raised / c.goal) * 100 : 0),
            0,
  ...
  const handleGenerateReport = async () => {
    ...
    setTimeout(() => {
      setGeneratedReport(
        `report_${Date.now()}.${reportConfig.delivery.format}`,
      );
  ...
  const handleDownloadReport = () => {
    ...
    const file = new Blob(["Mock report content..."], {
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start gap-4">
        <div>
          ...
          <p className="text-slate-600">
            Create professional PDF reports for stakeholders and board
            presentations
          </p>
        </div>
                        ...
                        <input
                          ...
                          onChange={(e) => {
                            const newStatus = e.target.checked
                              ...
                              : reportConfig.filters.status.filter(
                                  (s) => s !== status,
                                );
                            setReportConfig((prev) => ({
                              ...prev,
              ...
              <div className="bg-slate-50 rounded-lg p-6 border-2 border-dashed border-slate-300">
                <div className="text-center mb-6">
                  <h2 className="text-xl font-bold text-slate-900">
                    {reportConfig.customizations.title}
                  </h2>
                  <p className="text-slate-600">
                    {reportConfig.customizations.subtitle}
                  </p>
                </div>
                    ...
                    <div className="space-y-2">
                      ...
                      {reportStats.filteredCampaigns.length > 3 && (
                        <div className="text-xs text-slate-500">
                          +{reportStats.filteredCampaigns.length - 3} more
                          campaigns...
                        </div>
                      )}
                    </div>
            ...
            <div className="space-y-6">
              ...
              <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                ...
                <div className="space-y-1 text-sm text-blue-800">
                  ...
                  <div>
                    Format: {reportConfig.delivery.format.toUpperCase()}
                  </div>
      ...
      {currentStep === 4 && generatedReport && (
        <div className="space-y-6">
          <div className="text-center">
            ...
            <p className="text-slate-600">
              Your {selectedTemplate.name.toLowerCase()} is ready for download
            </p>
          </div>
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-slate-500">
                    {reportConfig.delivery.format.toUpperCase()} •{" "}
                    {selectedTemplate.estimatedPages} pages • Generated{" "}
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="bg-green-50 border border-green-200 rounded-lg p-4">
              ...
              <ul className="space-y-1 text-sm text-green-700">
                ...
                <li>
                  • {reportStats.totalDonors.toLocaleString()} donor records
                </li>
            ...
            <div className="bg-purple-50 border border-purple-200 rounded-lg p-4">
              ...
              <div className="text-sm text-purple-700">
                {reportConfig.delivery.schedule !== "now" ? (
                  <div>
                    <p>
                      This report is scheduled to generate{" "}
                      {reportConfig.delivery.schedule}
                    </p>
